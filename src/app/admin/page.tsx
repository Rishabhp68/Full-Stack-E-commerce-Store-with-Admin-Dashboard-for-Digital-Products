...
async function getUserData() {
    const [userCount, orderData] = await Promise.all([db.user.count(),
        db.order.aggregate({
            _sum: {pricePaidInCents: true}
        })
    ]);
...
export default async function AdminDashboard() {
    ...
    return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <DashboardCard title="Active Products" subtitle={`${formatNumber(productData.inactiveCount)} Inactive Products`} body={formatNumber(productData.activeCount)}></DashboardCard>
    </div>
}
